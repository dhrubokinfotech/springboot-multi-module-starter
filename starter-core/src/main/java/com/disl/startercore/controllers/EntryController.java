package com.disl.startercore.controllers;
...
import com.disl.startercore.entities.RefreshToken;
...
import com.disl.startercore.models.requests.*;
import com.disl.startercore.security.AuthUserInfoProviderURLFactory;
import com.disl.startercore.security.CustomUserDetailsService;
import com.disl.startercore.security.JwtTokenProvider;
import com.disl.startercore.security.OAuth2UserInfoFactory;
import com.disl.startercore.services.MailService;
import com.disl.startercore.services.RefreshTokenService;
import com.disl.startercore.services.UserService;
import com.disl.commons.exceptions.ResponseException;
import com.disl.startercore.models.responses.TokenResponse;
...
public class EntryController {
    ...
    public ResponseEntity<Response> createUser (@RequestBody SignUpRequest signUpRequest) {
		...
		return Response.getResponseEntity(true, "User Created. Please check your email address and verify your accounts");
    }
	...
	public ResponseEntity<Response> requestForgetPassword(@RequestBody InitialForgetPasswordRequest initialForgetPasswordRequest) {
		...
		if (CommonUtils.isValidMail(user.getEmail())) {
			String token = UUID.randomUUID().toString();
			user.setPasswordResetToken(token);
			userService.saveUser(user);

			if(appProperties.getActiveProfile() != CommonConstants.environment.development) {
				mailService.sendMail(user.getEmail(),
						AppConstants.FORGET_PASSWORD_SUBJECT, AppConstants.FORGET_PASSWORD_TEXT + appProperties.getBackendUrl()+AppConstants.RESET_PASSWORD_SUBURL + token
				);
			}
